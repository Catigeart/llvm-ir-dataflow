...
public:
    ...
    ///
    /// Merge of two dfvals, dst will be ther merged result
    /// @return true if dst changed
    ///
    virtual void merge(T *dst, const T &src ) = 0;
};
...
void compForwardDataflow(Function *fn,
    ...
    while (!worklist.empty()) {
        ...
        DEBUG_LOG("Function: " << bb->getParent()->getName() << " Basic block: " << bb->getName());
        DEBUG_LOG("Entry values: \n" << (*result)[bb].first);
